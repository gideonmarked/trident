...
function trident_login_redirect($redirect_to, $request, $user) {
    if (isset($user->roles) && is_array($user->roles)) {
        if (in_array('trident-admin', $user->roles) || in_array('administrator', $user->roles)) {
...
function trident_product_details_callback($post) {
    ...
    echo '<td>';
    $color_options = array('yellow', 'green', 'black', 'mint', 'gold');
...
function trident_add_product_ajax() {
    ...
    $body_color = sanitize_text_field($_POST['body_color']);
    $cap_color = sanitize_text_field($_POST['cap_color']);
    $boot_color = sanitize_text_field($_POST['boot_color']);
    ...
    $product_data = array(
        ...
        'image_url' => $image_url,
        'body_colors' => json_encode(array($body_color)),
        'cap_colors' => json_encode(array($cap_color)),
        'boot_colors' => json_encode(array($boot_color)),
